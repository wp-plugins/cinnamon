...
Plugin URI: http://www.artiss.co.uk/cinnamon
...
function cinnamon_set_plugin_meta( $links, $file ) {
	...
	if ( strpos( $file, 'cinnamon.php' ) !== false ) {
		$links = array_merge( $links, array( '<a href="http://wordpress.org/support/plugin/cinnamon">' . __( 'Support' ) . '</a>' ) );
...
function cinnamon_add_script() {
    ...
    wp_enqueue_script( 'cinnamon_script', WP_PLUGIN_URL . '/' . str_replace( basename( __FILE__ ), '', plugin_basename( __FILE__ ) ) . 'js/cinnamon.min.js', '', '', true );
